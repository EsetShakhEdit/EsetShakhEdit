local Ayatol_Korsi = "کسگم بازی در نیار به چیزی دس نزن😁👌 سخن بزرگان: REZA "
...
print("\027[" .. color.black[1] .. ";" .. color.blue[2] .. "m\n➡➡ [••ربات شاخ ادیت ایست برترین سورس ضد اسپم🔥••] ▶\n\027[00m")

print("\027[" .. color.white[1] .. ";" .. color.red[2] .. "m\n➡➡ [••درس زندگی=کونی باشی کیونت میزارن💦]  |          [••😒😒😒هر هر هر هر هر😒😒😒••] ▶\n\027[00m")

print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m\n➡➡ [••ربات api شاخ ادیت ایست فعال شد •فهرست• رو تو گروهت ارسال کن😒🖕] ▶\n\027[00m")

print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m\n➡➡ [••همچنین میتونی با ارسال Menu فهرست ربات api رو دریافت کنی••] ▶\n\027[00m")
...
local having_access = function(user_id, chat, Q_id)
  ...
  if not is_momod(user_id, chat) then
    if database:get("lang:gp:" .. chat) then
      ...
      Canswer(Q_id, "کصکش جقی دست نزن اه🤞😑")
    end
  end
  return var
end
        ...
        if msg.inline_query then
          ...
          if Q.query:match("^-(%d+,%S+)") then
            ...
            if lang == "Fa" then
              ...
              answer(Q.id, "settings", "Menu", chat, "⬇️بمال رو یڪے از دڪمـہ هاے زیر💦", keyboard)
            elseif lang == "En" then
              keyboard.inline_keyboard = {
                {
                  {
        ...
        if msg.callback_query then
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("FirstMenuFa") then
              ...
              edit(Q.inline_message_id, "⬇️بمال رو یڪے از دڪمـہ هاے زیر💦", keyboard)
